...
def lemmatize(sentIn):
    lemmatizer = WordNetLemmatizer()
    newS = ''
    sentIn = sentIn.split()
    for word in sentIn:
        newS = newS + lemmatizer.lemmatize(word) + ' '
    return newS

def makeNgram(tokenIn, ngrams):
    tokenOut = []
    for i in tokenIn:
        i = i.split()
        if(len(i)) > ngrams:
            for j in range(len(i) - (ngrams) + 1):
                tokenOut.append(' '.join(i[j:j + ngrams]))
        else:
            tokenOut.append(' '.join(i))
    return tokenOut
